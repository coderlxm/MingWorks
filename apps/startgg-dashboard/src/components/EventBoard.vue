...
const props = defineProps<{ detail: Detail; seedCount: number; canManage: boolean; stale: boolean }>()
...
const selectedPlayerName = computed(() => props.detail.players.find(player => String(player.entrantId) === String(route.query.entrant))?.playerName)
  ...
  <div class="event-board">
    ...
    <div v-show="tab === 'matches' || tab === 'players'" class="event-columns" :class="{ 'players-only': tab === 'players' }">
      <div v-show="tab === 'matches'" class="match-column">
        <ResultsFeed ref="results" :event-id="detail.event.eventId!" :recent="detail.recentSets" :live="detail.liveSets" :followed-entrants="followed" :selected-player-name="selectedPlayerName" :stale="stale" />
      </div>
