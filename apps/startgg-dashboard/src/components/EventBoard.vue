...
const props = defineProps<{ detail: Detail; seedCount: number }>()
...
const finalMatches = computed(() => [...props.detail.liveSets, ...props.detail.recentSets].filter(match => match.sources.includes('final')))
  ...
  <div class="event-board">
    ...
    <div v-show="tab === 'matches' || tab === 'players'" class="event-columns" :class="{ 'players-only': tab === 'players' }">
      <div v-show="tab === 'matches'" class="match-column">
        <section class="panel"><div class="section-heading"><h2><span class="live">●</span> 正在进行</h2><span class="count">{{ detail.liveSets.length }}</span></div><div class="match-stack"><MatchCard v-for="match in detail.liveSets" :key="match.setId" :match="match" :followed-entrants="followed" /></div><p v-if="!detail.liveSets.length" class="empty">当前采集范围内暂无进行中对局</p></section>
        <ResultsFeed ref="results" :event-id="detail.event.eventId!" :recent="detail.recentSets" :followed-entrants="followed" />
      </div>
