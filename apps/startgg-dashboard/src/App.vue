...
const { authenticated, board, following, details, detailErrors, error, actionError, operation, busy, sessionError, sessionSubmitting, sessionGeneration } = data
...
<template>
  <div class="app-shell">
    ...
    <div class="workspace">
      <aside class="sidebar" :class="{ 'mobile-open': mobileNav }" @click="($event.target as HTMLElement).closest('a') && (mobileNav = false)"><EventNavigator :events="board?.events ?? []" :selected="eventId" :pending-count="board?.pendingCount ?? 0" :can-manage="canManage" @manage="open('pending')" /></aside>
      <main class="main-content">
        ...
        <Overview v-else :board="board" :can-manage="canManage" @discover="open('discover')" />
        <KeepAlive :max="12"><EventBoard v-if="eventId && detail && !detailErrors[eventId]" :key="eventId" :detail="detail" :seed-count="following?.featuredSeedCount ?? 0" :can-manage="canManage" /></KeepAlive>
